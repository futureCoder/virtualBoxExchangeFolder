...
/**
 * 这个方法里把服务自己在C语言层面的回调函数给注销了，使它不再接收消息
 * 目的是：在lua层重新注册它，把消息通过lua接口来接收
 */
static int
launch_cb(struct skynet_context * context, void *ud, int type, int session, uint32_t source , const void * msg, size_t sz) {
	assert(type == 0 && session == 0);
	struct snlua *l = ud;
	skynet_callback(context, NULL, NULL);		//将服务原本绑定的句柄和回调函数清空
	int err = init_cb(l, context, msg, sz);		//设置各项资源路径参数，并加载 loader.lua
	if (err) {
 ...
 * 当此服务的消息队列被push进全局的消息队列后，本服务收到的第一条消息就是上述在初始化中给自己发送的那条消息
 * 此时便会调用回调函数launch_cb并执行处理逻辑
 */
int
snlua_init(struct snlua *l, struct skynet_context *ctx, const char * args) {
	int sz = strlen(args);
	char * tmp = skynet_malloc(sz);
	memcpy(tmp, args, sz);					//内存拷贝：将args内容拷贝到内存中的tmp指针指向地址的内存空间
	skynet_callback(ctx, l , launch_cb);	//注册回调函数为launch_cb这个函数，有消息传入时会调用回调函数并处理
	const char * self = skynet_command(ctx, "REG", NULL);
	uint32_t handle_id = strtoul(self+1, NULL, 16);		//当前lua实例自己的句柄id（转为无符号长整型）
	// it must be first message
	skynet_send(ctx, 0, handle_id, PTYPE_TAG_DONTCOPY,0, tmp, sz);		// 给自己发送一条消息，内容为args字符串
	return 0;
}
...
//snlua 是lua的沙盒服务，所有的 lua服务 都是一个 snlua 的实例。
struct snlua *
snlua_create(void) {
