...
struct skynet_config {
	//启动工作线程数量，不要配置超过实际拥有的CPU核心数
	int thread;

	//skynet网络节点的唯一编号，可以是 1-255 间的任意整数。一个 skynet 网络最多支持 255 个节点。
	//每个节点有必须有一个唯一的编号。如果 harbor 为 0 ，skynet 工作在单节点模式下。此时 master 和 address 以及 standalone 都不必设置。
	int harbor;

	//是否开启统计功能，统计每个服务使用了多少cpu时间，默认开启
	int profile;

	//后台模式：daemon = "./skynet.pid"可以以后台模式启动skynet（注意，同时请配置logger 项输出log）
	const char * daemon;
	...
	//注：如果你希望用 lua 来编写这个服务，可以在这里填写 snlua ，然后在 logger 配置具体的 lua 服务的名字。在 examples 目录下，有 config.userlog 这个范例可供参考。
