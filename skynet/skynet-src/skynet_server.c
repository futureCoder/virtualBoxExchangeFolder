 ...
 * 实例化一个服务
 * @param name - 加载模块的名称
 * @param param - 传入设置参数，初始化由模块生成的实例时所需
 * 
 */

struct skynet_context * 
skynet_context_new(const char * name, const char *param) {
	struct skynet_module * mod = skynet_module_query(name);		//会从 name(如logger.so) 中把模块加载出来

	if (mod == NULL)
		return NULL;

	void *inst = skynet_module_instance_create(mod);			//让加载出来的模块自动生成一个新的实例
	if (inst == NULL)
		return NULL;
	...
	ctx->handle = 0;	
	ctx->handle = skynet_handle_register(ctx);					//给新实例注册一个事件处理的handle
	struct message_queue * queue = ctx->queue = skynet_mq_create(ctx->handle);	//创建这个实例的消息队列
	// init function maybe use ctx->handle, so it must init at last
	context_inc();

	CHECKCALLING_BEGIN(ctx)
	int r = skynet_module_instance_init(mod, inst, ctx, param);	//调用模块的初始化方法
